...
export async function updatePrices(
  ...
  const targetChain = options.chain ?? DEFAULT_CHAIN;
  ...
  const tokenAddresses = tokenRecords
  ...
  log.info(
    { chain: targetChain, tokens: tokenAddresses.length, batchSize },
  ...
  log.info(
    { chain: targetChain, updated: updates },
    "Price job completed",
  );

  return {
    chain: targetChain,
    processedTokens: tokenAddresses.length,
    updated: updates,
    timestamp,
  };
}
